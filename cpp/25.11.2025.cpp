// ============================================================
//  Иерархия транспорта — наследование + виртуальные методы
//  + фабричный интерфейс для создания объектов в рантайме
// ============================================================
#include <iostream>
#include <string>
#include <vector>
...
public:
    ...
    // ── Чистый виртуальный — каждый подкласс обязан определить ──
...
// ─── Промежуточные классы (средний уровень) ─────────────────
...
// ─── Фабричный интерфейс ─────────────────────────────────────
//
//  Чтобы создавать объекты на этапе выполнения без знания
//  конкретных типов в месте вызова — используем абстрактную
//  фабрику + статический реестр.

using TransportPtr    = std::unique_ptr<Transport>;
using FactoryFunction = TransportPtr(*)();

// Глобальный реестр: ключ → функция-фабрика
std::map<std::string, FactoryFunction> g_registry;

// Вспомогательная функция для безопасного ввода int
int read_int(const std::string& prompt, int min_val = 0, int max_val = 100000) {
    int val;
    while (true) {
        std::cout << prompt;
        if (std::cin >> val && val >= min_val && val <= max_val) break;
        std::cin.clear();
...
int main() {
    ...
    while (choice != 0) {
        ...
        case 1: {
            ...
            for (auto& ch : key) ch = static_cast<char>(std::tolower(
        ...
        case 2: {
            if (fleet.empty()) { std::cout << "Парк пуст.\n"; break; }
        ...
        case 3: {
            ...
            if      (cmd == "move") fleet[idx]->move();
            else if (cmd == "stop") fleet[idx]->stop();
            else if (cmd == "fuel") fleet[idx]->refuel();
